...
PatientHistoryWidget::PatientHistoryWidget(QWidget* parent) : QWidget(parent)
{
    ...
    auto* pristanakBtn = new QPushButton("📝 Otvori pristanak");
    ...
    consentBtn->setStyleSheet(btnStyle("#1565C0","#1976D2"));
    pristanakBtn->setStyleSheet(btnStyle("#6A1B9A","#7B1FA2"));

    actRow->addWidget(editBtn);
    actRow->addWidget(delBtn);
    actRow->addWidget(pdfBtn);
    actRow->addWidget(consentBtn);
    actRow->addWidget(pristanakBtn);
    ...
    connect(pristanakBtn, &QPushButton::clicked, this, &PatientHistoryWidget::onViewPristanakClicked);
...
QString PatientHistoryWidget::buildInterventionHtml(const Intervention& iv)
{
    ...
    // Upitnik o zdravlju badge
    if (!iv.healthPdfPath.isEmpty()) {
        html += "<div style='margin-bottom:6px;'>"
                "<span style='background:#E3F2FD;border-left:3px solid #1565C0;"
                "padding:4px 10px;border-radius:0 4px 4px 0;color:#1565C0;font-size:9pt;'>"
                "🏥 Upitnik o zdravlju priložen</span></div>";
    }

    // Pristanak na intervenciju badge
    if (!iv.consentPdfPath.isEmpty()) {
        html += "<div style='margin-bottom:6px;'>"
                ...
                "📝 Pristanak na intervenciju priložen</span></div>";
    }
...
void PatientHistoryWidget::onViewConsentClicked()
{
    int row = m_ivList->currentRow();
    if (row < 0 || row >= m_currentIvs.size()) return;
    const QString& path = m_currentIvs[row].healthPdfPath;
    if (path.isEmpty()) {
        QMessageBox::information(this, "Upitnik",
            "Ova intervencija nema priložen upitnik o zdravstvenom stanju.");
        return;
    }
    emit viewPdf(path);
}

void PatientHistoryWidget::onViewPristanakClicked()
{
    int row = m_ivList->currentRow();
    if (row < 0 || row >= m_currentIvs.size()) return;
    const QString& path = m_currentIvs[row].consentPdfPath;
    if (path.isEmpty()) {
        QMessageBox::information(this, "Pristanak",
            "Ova intervencija nema priložen pristanak na intervenciju.");
        return;
    }
    emit viewPdf(path);
}
